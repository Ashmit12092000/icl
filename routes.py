        ...
        try:
            ...
            if period_from and period_to:
                ...
                if should_use_simple_interest:
                    ...
                    # For compound interest, we need to calculate the accumulated balance including all previous net interest
                    accumulated_balance = Decimal('0')
                    
                    # Calculate accumulated balance from all previous transactions
                    for prev_txn in Transaction.query.filter(
                        Transaction.customer_id == customer_id,
                        Transaction.date < transaction_date
                    ).order_by(Transaction.date, Transaction.id).all():
                        accumulated_balance += prev_txn.get_safe_amount_paid() - prev_txn.get_safe_amount_repaid()
                        accumulated_balance += prev_txn.get_safe_net_amount()
                    
                    # Add same-day transactions that occurred before this one
                    same_day_previous_txns = Transaction.query.filter(
                        Transaction.customer_id == customer_id,
                        Transaction.date == transaction_date
                    ).order_by(Transaction.id).all()
                    
                    for same_day_txn in same_day_previous_txns:
                        accumulated_balance += same_day_txn.get_safe_amount_paid() - same_day_txn.get_safe_amount_repaid()
                        accumulated_balance += same_day_txn.get_safe_net_amount()
                    
                    if amount_paid > Decimal('0'):
                        # For deposits, interest is calculated on accumulated balance + new deposit
                        principal_for_interest_calculation = accumulated_balance + amount_paid
                    elif amount_repaid > Decimal('0'):
                        # For repayments, interest is calculated on the reduced principal (accumulated balance minus repayment)
                        principal_for_interest_calculation = accumulated_balance - amount_repaid
                    else:
                        # For passive periods, interest is calculated on accumulated balance
                        principal_for_interest_calculation = accumulated_balance

                if should_use_simple_interest:
                    int_amount = calculate_interest(principal_for_interest_calculation, customer.annual_rate, no_of_days)
                else:
                    # For compound interest, we need to calculate based on the accumulated balance
                    # including all previous principal and compounded interest
                    int_amount = calculate_interest(principal_for_interest_calculation, customer.annual_rate, no_of_days)
        ...
        for transaction in transactions_to_recalculate:
            ...
            if should_use_simple_interest:
                ...
                if transaction.get_safe_amount_paid() > Decimal('0'):
                    ...
                    # Repayment — interest on the reduced principal (accumulated balance minus repayment)
                    principal_for_interest_calculation = current_running_balance - transaction.get_safe_amount_repaid()
                    logging.debug(f"  Compound Interest - Repayment - Principal for interest (reduced principal): {principal_for_interest_calculation}")
                else:
                    # Passive period — interest on accumulated balance
                    principal_for_interest_calculation = current_running_balance
                    logging.debug(f"  Compound Interest - Passive - Principal for interest (accumulated balance): {principal_for_interest_calculation}")
